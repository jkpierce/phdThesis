...
This joint density is completely determined from the solutions of eqs. \ref{eq:g1}-\ref{eq:g2} given specification of the distributions $g_{ij}$ and $G_i$.
...
Equations \ref{eq:g1}-\ref{eq:g2} with these $g_{ij}$ and $G_i$ are solved using Laplace transforms in space and time $(x,t \rightarrow 
...
Figure \ref{fig:diffpdfs} depicts the distribution (eq. \ref{eq:pdf}) alongside simulations generated by a direct method based on evaluating the cumulative transition probabilities between states on a small timestep \citep{Barik2006}.
...
This work provides a complementary perspective on the three range diffusion exhibited in ch. \ref{ch:flux}.
