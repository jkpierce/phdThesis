...
%DIFDELCMD < %%%
\DIFdel{The Gillespie Stochastic Simulation Algorithm }\DIFdelend \DIFaddbegin \label{sec:arr}
\DIFadd{The Gillespie stochastic simulation algorithm }\DIFaddend (SSA) generates exact realizations of a Markov random process from a sequence of random numbers.
...
The SSA hinges on the defining property of a Markov process. When the transition rates from one state to another are not dependent on the distant past, the process is Markov \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep[e.g.,][]{Cox1965}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Cox1965}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
The following sections demonstrate that the time intervals $\tau$ between subsequent transitions are exponentially distributed within the model of \DIFdelbegin \DIFdel{chapter }\DIFdelend \DIFaddbegin \DIFadd{Ch. }\DIFaddend \ref{ch:ch3}. The SSA follows as a consequence of this property.
...
The description involves four possible transitions (migration in, entrainment, deposition, migration out) with rates given in \DIFdelbegin \DIFdel{equations (}\DIFdelend \DIFaddbegin \DIFadd{Eqs. }\DIFaddend \ref{eq:rate1}-\ref{eq:rate4}\DIFdelbegin \DIFdel{)}\DIFdelend .
...
\DIFaddend So far, \DIFdelbegin \DIFdel{equation }\DIFdelend \DIFaddbegin \DIFadd{Eq. }\DIFaddend \ref{eq:exp} demonstrates how to step the time from one transition to the next, but it remains unclear how to step the state variables $n$ and $m$ at each transition time. There is a need to characterize the type of transition which occurs at a given transition time.

Intuitively, this will depend on the relative magnitudes of the rates from \DIFdelbegin \DIFdel{equations (}\DIFdelend \DIFaddbegin \DIFadd{Eqs. }\DIFaddend \ref{eq:rate1}-\ref{eq:rate4}\DIFdelbegin \DIFdel{)}\DIFdelend .
...
In summary, to step the process through a single transition, the SSA draws the time interval to the next transition from the distribution (\DIFaddbegin \DIFadd{Eq. }\DIFaddend \ref{eq:exp}), draws a uniform random \DIFaddbegin \DIFadd{number }\DIFaddend from $[0,1]$, then uses this uniform random to select the transition that occurs from the cumulative sum of the ratios in \DIFdelbegin \DIFdel{(\ref{eq:rel})}\DIFdelend \DIFaddbegin \DIFadd{Eq. \ref{eq:rel}}\DIFaddend . Iterating this random number generation and selection process simulates exact realizations of the stochastic process. These are series of $n$ and $m$ through time from which any statistics of interest can be calculated.

\subsection{\DIFdelbegin \DIFdel{Pseudo code }\DIFdelend \DIFaddbegin \DIFadd{Pseudocode }\DIFaddend for the Gillespie SSA}
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{sec:crr}
\DIFaddend Simulations are initialized by the initial conditions $n_0$ and $m_0$, the model parameters used in \DIFdelbegin \DIFdel{equations }\DIFdelend \DIFaddbegin \DIFadd{Eqs. }\DIFaddend \ref{eq:rate1}-\ref{eq:rate4}, and the desired simulation duration $t_\text{max}$. The SSA uses these inputs to generate \DIFdelbegin \DIFdel{time series }\DIFdelend \DIFaddbegin \DIFadd{timeseries }\DIFaddend of $n$ and $m$ as follows:
...
\begin{algorithmic}
	...
	\DIFdelbegin %DIFDELCMD < \While{$t<t_{\text{max}}$;}{	\Comment{Simulation will go until $t$ surpasses $t_\text{max}$}
%DIFDELCMD < 		\State record $(n,m,t)$ \Comment{Build time series of $n$ and $m$}		
%DIFDELCMD < 		\State draw $\tau$ from eq. (\ref{eq:exp}) \Comment{Select time to next transition}
%DIFDELCMD < 		\State $t \coloneqq t+\tau$
%DIFDELCMD < 		\State draw a random number $r$ in $[0,1]$ \Comment{Select type of transition that occurs}
%DIFDELCMD < 		\State compute the ratios $r_1,r_2,r_3,r_4$ in eq. (\ref{eq:rel})
%DIFDELCMD < 		\State form the cumulative sums $r_i = \sum_{1\leq j\leq i}r_j$ 
%DIFDELCMD < 		\Comment{Now enact the transition}
%DIFDELCMD < 		\If {$0\leq r<r_1$}	\Comment{Migration in}
%DIFDELCMD < 		\State $n \coloneqq n+1$
%DIFDELCMD < 		\ElsIf {$r_1\leq r < r_2$} 	\Comment{Entrainment}
%DIFDELCMD < 		\State $n \coloneqq n + 1$
%DIFDELCMD < 		\State $m \coloneqq m-1 $
%DIFDELCMD < 		\ElsIf {$r_2\leq r < r_3$} 	\Comment{Deposition}
%DIFDELCMD < 		\State $ n \coloneqq n - 1$
%DIFDELCMD < 		\State $ m \coloneqq m + 1$
%DIFDELCMD < 		\ElsIf {$r_3 \leq r \leq 1$} \Comment{ Migration out}
%DIFDELCMD < 		\State $ n \coloneqq n-1$
%DIFDELCMD < 		\EndIf
%DIFDELCMD < 	}
%DIFDELCMD < 	%%%
\DIFdelend \DIFaddbegin \While{$t<t_{\text{max}}$;}{	\Comment{Simulation will go until $t$ surpasses $t_\text{max}$}
...
\section{Approximate solutions of the \DIFdelbegin \DIFdel{Master }\DIFdelend \DIFaddbegin \DIFadd{master }\DIFaddend equation}

\subsection{Mean field solution of particle activity}

\DIFdelbegin \DIFdel{By assuming }\DIFdelend \DIFaddbegin \DIFadd{Assuming }\DIFaddend the dynamics of the particle activity are totally independent of the bed elevation and summing \DIFdelbegin \DIFdel{equation }\DIFdelend \DIFaddbegin \DIFadd{Eq. }\DIFaddend \ref{eq:elemaster} over all values of $m$ obtains a mean field equation for the particle activity: 
...
From the definition of $G$ it follows that $A(n) = \frac{1}{n!} \frac{d^nG}{dx^n}|_{x=0}$, giving the negative binomial distribution of particle activity demonstrated by \citet{Ancey2008}\DIFdelbegin \DIFdel{and stated in equation \ref{eq:ancey}}\DIFdelend \DIFaddbegin \DIFadd{, stated in Eq. \ref{eq:ancey}, }\DIFaddend and introduced originally in \DIFdelbegin \DIFdel{section }\DIFdelend \DIFaddbegin \DIFadd{Sec. }\DIFaddend \ref{sec:birthdeath}.
...
\DIFaddend The negative binomial distribution provides $\langle n |m \rangle = \langle n \rangle$, so \DIFdelbegin \DIFdel{equation }\DIFdelend \DIFaddbegin \DIFadd{Eq. }\DIFaddend \ref{eq:elemaster} produces
...
Identifying $E=\lambda + \mu \langle n \rangle$ and $D = \sigma \langle n \rangle$ and incorporating $E=D$ gives \DIFdelbegin \DIFdel{equation }\DIFdelend \DIFaddbegin \DIFadd{Eq. }\DIFaddend \ref{eq:ou}:
\be 0 = [1+\kappa(m+1)]M(m+1) + [1-\kappa(m-1)]M(m-1) - 2M(m).\ee

This can be solved using the Fokker-Planck expansion \citep{Gardiner1983} that effectively converts this discrete \DIFdelbegin \DIFdel{Master }\DIFdelend \DIFaddbegin \DIFadd{master }\DIFaddend equation for $m$ into a diffusion equation for the quasi-continuous variable $z=z_1 m$\DIFaddbegin \DIFadd{. }\DIFaddend This works since $z_1$ is small. Introducing $z$ and writing $\overline{\kappa}=\kappa/z_1$ gives
...
as provided in \DIFdelbegin \DIFdel{chapter }\DIFdelend \DIFaddbegin \DIFadd{Ch. }\DIFaddend \ref{ch:ch3}.
...
for the mean particle activity conditional to the elevation $m$ into \DIFaddbegin \DIFadd{Eq. }\DIFaddend \ref{eq:elemaster}, noting $E=D$, and neglecting terms of $O(\kappa^2)$ provides 
...
\be M(m) = M_0 \exp\Big\{-\kappa m^2 \Big[1 + \frac{(\sigma-\mu)c}{2E}\Big]\Big\}. \DIFaddbegin \label{eq:zap}\DIFaddend \ee
Since the numerical solutions show $\sigma_m^2 = \frac{1}{4\kappa}$, this suggests the closure relation
\be \langle n | m \rangle = \langle n \rangle\Big( 1-\frac{2\kappa m}{1-\mu/\sigma}\Big) \DIFaddbegin \label{eq:zim}\DIFaddend \ee
corresponding to $c=2E/(\sigma-\mu)$. This is the closure relationship provided in \DIFdelbegin \DIFdel{equation }\DIFdelend \DIFaddbegin \DIFadd{Eq. }\DIFaddend \ref{eq:closure} and plotted with the numerical simulations in \DIFdelbegin \DIFdel{figure \ref{fig:condmoms}}\DIFdelend \DIFaddbegin \DIFadd{Fig. \ref{fig:condmoms}.
The elevation distribution Eq. \ref{eq:ou2} follows by combining Eqs. \ref{eq:zap} and \ref{eq:zim}}\DIFaddend .
