...
With the scaling $\sigma_x^2 \propto t$, the diffusion is said to be normal, since this is found in the classic problems \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{Taylor1920}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Einstein1905}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
...
At long timescales, a significant fraction of tracers \DIFdelbegin \DIFdel{bury }\DIFdelend \DIFaddbegin \DIFadd{become buried }\DIFaddend under the bed surface \citep{Hassan1991,Hassan2013,Ferguson2002a,Haschenburger2013,Papangelakis2016}, meaning burial dominates long term diffusion characteristics \citep{Bradley2017,Martin2014,Voepel2013}, possibly at global or even longer ``geomorphic'' timescales \citep{Hassan2017} than Nikora et al. originally considered.
...
Among these, \citet{Nikora2001a} used synthetic turbulence \citep{Kraichnan1970} with a discrete element method for the granular phase \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{Cundall1979a}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Cundall1979}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend ; \citet{Bialik2012} used synthetic turbulence with a random collision model \citep{Sekine1992}; and \citet{Fan2016} used a Langevin equation with probabilistic rests.
To \DIFdelbegin \DIFdel{our }\DIFdelend \DIFaddbegin \DIFadd{my }\DIFaddend knowledge, only \citet{Bialik2015} have claimed to capture all three ranges from a Newtonian approach.
...
These earlier works suggest the minimal required components to model three bedload diffusion ranges \DIFdelbegin \DIFdel{: }\DIFdelend \DIFaddbegin \DIFadd{are }\DIFaddend (1) exchange between motion and rest intervals and (2) the sediment burial process.
...
Below, I develop and solve the model in \DIFdelbegin \DIFdel{section }\DIFdelend \DIFaddbegin \DIFadd{Sec. }\DIFaddend \ref{sec:model}. Then, I discuss the predictions of this model, present its implications for local, intermediate, and global ranges of bedload diffusion, and suggest next steps for bedload diffusion research in \DIFdelbegin \DIFdel{sections }\DIFdelend \DIFaddbegin \DIFadd{Secs. }\DIFaddend \ref{sec:discussion} and \ref{sec:conclusion}.

\section{Bedload trajectories as a multi-state random walk}
\label{sec:model}
\subsection{Assumptions of the burial model}
\label{sec:assumptions}
Particle trajectories are formulated as a three-state random walk where the states are motion, surface rest, and burial. These states are \DIFdelbegin \DIFdel{labelled }\DIFdelend \DIFaddbegin \DIFadd{labeled }\DIFaddend as $i=2$ (motion), $i=1$ (rest), and $i=0$ (burial).
...
Grains in motion are considered to have a characteristic velocity \DIFdelbegin \DIFdel{$V$ }\DIFdelend \DIFaddbegin \DIFadd{$v$ }\DIFaddend \citep{Lisle1998,Lajeunesse2017}, and burial is \DIFdelbegin \DIFdel{modelled }\DIFdelend \DIFaddbegin \DIFadd{modeled }\DIFaddend as long lasting enough to be effectively permanent \citep{Wu2019}, with grains resting on the surface having a probability per unit time $\kappa$ to become buried.
...
Denoting by $g_{ij}(x,t)$ the probability for a particle to \DIFdelbegin \DIFdel{displace by }\DIFdelend \DIFaddbegin \DIFadd{move a distance }\DIFaddend $x$ in a time $t$ within the state $i$ before it transitions to the state $j$, the transition probabilities $\om_{ij}(x,t)$ sum over all possible paths to the state $i$ from previous locations and times:
\be \om_{ij}(x,t) = \theta_i g_{ij}(x,t) + \sum_{k=0}^2 \int_0^x dx' \int_0^t dt' \om_{ki}(x',t')g_{ij}(x-x',t-t').\label{eq:g1}\ee
Defining another set of probabilities $G_i(x,t)$ that a particle \DIFdelbegin \DIFdel{displaces }\DIFdelend \DIFaddbegin \DIFadd{moves }\DIFaddend by a distance $x$ in a time $t$ within the state $i$ and possibly remains within the state, a similar sums over paths for the probabilities to be in the state $i$ at $x$, $t$ produces: 
...
\be p(x,t) = \sum_{k=0}^2 p_k(x,t)\DIFaddbegin \DIFadd{. }\DIFaddend \ee
This joint density is completely determined from the solutions of \DIFdelbegin \DIFdel{equations (\ref{eq:g1} -\ref{eq:g2} ) }\DIFdelend \DIFaddbegin \DIFadd{Eqs. \ref{eq:g1} and \ref{eq:g2} }\DIFaddend given specification of the distributions $g_{ij}$ and $G_i$.


\subsection{\DIFdelbegin \DIFdel{Joint probability }\DIFdelend \DIFaddbegin \DIFadd{Probability }\DIFaddend distribution of particle position with burial}
\label{sec:solution}

These distributions can be constructed from the assumptions described in \DIFdelbegin \DIFdel{section }\DIFdelend \DIFaddbegin \DIFadd{Sec. }\DIFaddend \ref{sec:assumptions}.
Since particles resting on the bed surface \DIFdelbegin \DIFdel{bury }\DIFdelend \DIFaddbegin \DIFadd{become buried }\DIFaddend in a time $t$ with probability $\Phi(t)$, and resting times are distributed as $\psi_1(t)$, these assumptions obtain $g_{12}(x,t) = \delta(x)k_1e^{-k_1t}e^{-\kappa t}$ and $g_{10}(x,t) = \delta(x) k_1 e^{-k_1 t}(1-e^{-\kappa t})$. Since motions have velocity $v$ for times distributed as $\psi_2(t)$, the assumptions provide $g_{21}(x,t) = \delta(x-vt)k_2e^{-k_2 t}$.
...
\DIFdelbegin \DIFdel{Equations (\ref{eq:g1} -\ref{eq:g2} ) }\DIFdelend \DIFaddbegin \DIFadd{Eqs. \ref{eq:g1} and \ref{eq:g2} }\DIFaddend with these $g_{ij}$ and $G_i$ are solved using Laplace transforms in space and time $(x,t \rightarrow 
...
where $k' = k_1 + k_2$. \DIFdelbegin \DIFdel{Inverting }\DIFdelend \DIFaddbegin \DIFadd{As explained in appendix Sec. \ref{sec:appendixA}, inverting }\DIFaddend this result using known Laplace transforms \citep{Prudnikov1992a,Arfken1985} obtains
\begin{align}
	\begin{split}
...
\begin{figure}[!htbp]
	...
	\caption{Joint distributions for a grain to be at position $x$ at time $t$ are displayed for the choice $k_1=0.1$, $k_2=1.0$, $v=2.0$. Grains are \DIFdelbeginFL \DIFdelFL{considered }\DIFdelendFL initially at rest ($\theta_1=1$, $\theta_2=0$). The solid lines are the analytical distribution in \DIFdelbeginFL \DIFdelFL{equation (}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Eq. }\DIFaddendFL \ref{eq:pdf}), while the points are numerically simulated, showing the correctness of \DIFdelbeginFL \DIFdelFL{our }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{the mathematical }\DIFaddendFL derivations. Colors pertain to different times. Units are unspecified, since the aim is to demonstrate the general characteristics of $p(x,t)$. Panel (a) shows the case $\kappa=0$ -- no burial. In this case, the joint distribution tends toward Gaussian at large times \citep{Einstein1937,Lisle1998}. Panel (b) shows the case when grains have rate $\kappa = 0.01$ to become buried while resting \DIFaddbeginFL \DIFaddFL{on the bed surface}\DIFaddendFL . Because of burial, the joint distribution tends toward a more uniform distribution than Gaussian.
		\label{fig:diffpdfs}}
\end{figure}

\DIFdelbegin \DIFdel{Figure }\DIFdelend \DIFaddbegin \DIFadd{Fig. }\DIFaddend \ref{fig:diffpdfs} depicts the distribution \DIFdelbegin \DIFdel{(\ref{eq:pdf} ) }\DIFdelend \DIFaddbegin \DIFadd{Eq. \ref{eq:pdf} }\DIFaddend alongside simulations generated by a direct method based on evaluating the cumulative transition probabilities between states on a small timestep \citep{Barik2006}.
When grains do not become buried, as in panel (a) of \DIFdelbegin \DIFdel{figure }\DIFdelend \DIFaddbegin \DIFadd{Fig. }\DIFaddend \ref{fig:diffpdfs}, the distribution becomes Gaussian-like at relatively large observation times, exemplifying normal diffusion and satisfying the central limit theorem.
When grains become buried, as in panel (b) of \DIFdelbegin \DIFdel{figure }\DIFdelend \DIFaddbegin \DIFadd{Fig. }\DIFaddend \ref{fig:diffpdfs},  the Q-function terms prevent the distribution from approaching a Gaussian at large timescales, exemplifying anomalous diffusion \citep{Weeks1998} and violating the central limit theorem \citep{Metzler2000,Schumer2009}.
...
In these equations, $a = (\kappa + k_1+k_2)/2$ and $b = \sqrt{a^2-\kappa k_2}$ are effective rates having dimensions of inverse time, while the $A$, $B$, and $C$ factors are provided in \DIFdelbegin \DIFdel{table }\DIFdelend \DIFaddbegin \DIFadd{Tab. }\DIFaddend \ref{table:params}.
...
\begin{table}[!h]
	...
	\caption{Abbreviations used in the expressions of the mean (\DIFaddbeginFL \DIFaddFL{Eq. }\DIFaddendFL \ref{eq:mean}), second moment (\DIFaddbeginFL \DIFaddFL{Eq. }\DIFaddendFL \ref{eq:second}) and variance (\DIFaddbeginFL \DIFaddFL{Eq. }\DIFaddendFL \ref{eq:var}) of bedload tracers.}
	\label{table:params}
	\small
	\begin{tabular}{c}
		\toprule
	...
	\vspace{-0.5cm}
\end{table}
The positional variance \DIFdelbegin \DIFdel{(\ref{eq:var} ) }\DIFdelend \DIFaddbegin \DIFadd{Eq. \ref{eq:var} }\DIFaddend is plotted in \DIFdelbegin \DIFdel{figure }\DIFdelend \DIFaddbegin \DIFadd{Fig. }\DIFaddend \ref{fig:diffvar} for conditions $\theta_1=1$ and $k_2\gg k_1 \gg \kappa$.
...
\DIFdelbegin \DIFdel{Figure }\DIFdelend \DIFaddbegin \DIFadd{Fig. }\DIFaddend \ref{fig:diffvar} demonstrates that under these conditions the variance \DIFdelbegin \DIFdel{(\ref{eq:var} ) }\DIFdelend \DIFaddbegin \DIFadd{\ref{eq:var} }\DIFaddend shows three diffusion ranges with approximate power law scaling ($\sigma_x^2 \propto t^\gamma$) that are identified as the local, intermediate, and global ranges proposed by Nikora et al., followed by a fourth range of no diffusion ($\sigma_x^2 = \text{const}$) stemming from the burial of all tracers. 
Following \citet{Hassan2017} I suggest to call the fourth range \DIFdelbegin \DIFdel{geomorphic}\DIFdelend \DIFaddbegin \DIFadd{"geomorphic"}\DIFaddend , since any further transport in this range can occur only if scour re-exposes buried grains to the flow \citep{Nakagawa1980,Voepel2013,Martin2014,Wu2019a}.

\begin{figure}[!htbp]	
	\includegraphics[width=\linewidth,keepaspectratio]{./figures/ch4/diffusion.png}
	\caption{Panel (a) sketches conceptual trajectories of \DIFdelbeginFL \DIFdelFL{three }\DIFdelendFL grains, while panel (b) depicts \DIFdelbeginFL \DIFdelFL{the variance (}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Eq. }\DIFaddendFL \ref{eq:var} \DIFdelbeginFL \DIFdelFL{) }\DIFdelendFL with mean motion time $1.5$s, \DIFdelbeginFL \DIFdelFL{resting }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{rest }\DIFaddendFL time $30.0$s, and \DIFdelbeginFL \DIFdelFL{movement }\DIFdelendFL velocity $0.1$m/s\DIFdelbeginFL \DIFdelFL{-- values comparable to laboratory experiments transporting small ($5$ mm) gravels \mbox{%DIFAUXCMD
\citep{Lajeunesse2010,Martin2012}}\hspace{0pt}%DIFAUXCMD
}\DIFdelendFL . The burial timescale is \DIFdelbeginFL \DIFdelFL{$7200.0$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$7200$}\DIFaddendFL s (two hours), and grains start from rest ($\theta_1=1$). The solid line is \DIFdelbeginFL \DIFdelFL{equation (}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Eq. }\DIFaddendFL \ref{eq:var} \DIFdelbeginFL \DIFdelFL{), }\DIFdelendFL and the points are numerically simulated. Panel (b) demonstrates four distinct scaling ranges of $\sigma_x^2$: local, intermediate, global, and geomorphic. The first three are diffusive. \DIFdelbeginFL \DIFdelFL{Three crossover }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Crossover }\DIFaddendFL times $\tau_L$, $\tau_I$, and $\tau_G$ divide the ranges. \DIFdelbeginFL \DIFdelFL{Within each range, a slope key demonstrates }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Slope keys demonstrate }\DIFaddendFL the scaling $\sigma_x^2 \propto t^\gamma$ \DIFaddbeginFL \DIFaddFL{in each range}\DIFaddendFL . Panel (a) demonstrates that different mixtures of motion, rest, and burial states generate the ranges. At local timescales, grains usually either rest or move; at intermediate timescales, they transition between rest and motion; at global timescales, they transition between rest, motion, and burial; and at geomorphic timescales, all grains \DIFdelbeginFL \DIFdelFL{bury. Additional slope keys in the local and global ranges of panel (b) illustrate the effect of initial conditions and rest/burial timescales on the diffusion, while the additional slope key within the geomorphic range demonstrates the expected scaling when burial is not permanent, as discussed in section \ref{sec:discussion}}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{become buried}\DIFaddendFL .
	}
	\label{fig:diffvar}
\end{figure}

\subsection{Diffusion exponents and \DIFdelbegin \DIFdel{three range }\DIFdelend \DIFaddbegin \DIFadd{three-range }\DIFaddend scaling}
\DIFaddbegin \label{sec:amidonenow}
\DIFaddend 

Two limiting cases of \DIFdelbegin \DIFdel{equation (\ref{eq:var} ) }\DIFdelend \DIFaddbegin \DIFadd{Eq. \ref{eq:var} }\DIFaddend provide the scaling exponents $\gamma$ of the diffusion $\sigma_x^2 \propto t^\gamma$ in each range. Limit (1) represents times so short \DIFaddbegin \DIFadd{that }\DIFaddend a negligible amount of sediment burial has occurred, $t\ll 1/\kappa$, while limit (2) represents times so long motion intervals appear as instantaneous steps of mean length $l=v/k_2$, $1/k_2 \rightarrow 0$ while $v/k_2 = \text{constant}$.
...
To summarize, when $k_2\gg k_1 \gg \kappa$ so all three diffusion ranges exist, \DIFdelbegin \DIFdel{equation (\ref{eq:var} ) }\DIFdelend \DIFaddbegin \DIFadd{Eq. \ref{eq:var} }\DIFaddend implies:
\begin{enumerate}
...
A generalization of the model in this chapter to include heavy-tailed timescales between burial and exhumation might develop four ranges of diffusion, where the long-time decay of the exhumation time distribution would dictate the geomorphic range diffusion characteristics as depicted in \DIFdelbegin \DIFdel{figure }\DIFdelend \DIFaddbegin \DIFadd{Fig. }\DIFaddend \ref{fig:diffvar}.
If cumulative exhumation times decay faster than $T^{-1/2}$, as suggested in \DIFdelbegin \DIFdel{chapter }\DIFdelend \DIFaddbegin \DIFadd{Ch. }\DIFaddend \ref{ch:ch3}, other equilibrium transport models \citep{Voepel2013, Martin2014}, and laboratory experiments \citep{Martin2014,Martin2012}, the geomorphic range is expected to be super-diffusive \citep{Weeks1998}.
...
\DIFdelbegin \DIFdel{I believe further }\DIFdelend \DIFaddbegin \DIFadd{Further }\DIFaddend developing the linkage between existing bedload models and random walk concepts could rapidly progress our understanding.
...
Pushing their ideas somewhat further, I followed \citet{Hassan2017} to propose a geomorphic range \DIFdelbegin \DIFdel{to describe }\DIFdelend \DIFaddbegin \DIFadd{describing particle }\DIFaddend diffusion characteristics at timescales larger than the global range when burial and exhumation both moderate downstream transport.
...
\DIFdelbegin \DIFdel{This work provides a complementary perspective on the three range diffusion exhibited in chapter \ref{ch:flux}.
}\DIFdelend A next step is to incorporate exhumation to better understand the geomorphic range.
Ultimately, the multi-state random walk formalism used in this chapter has been demonstrated to implicitly \DIFdelbegin \DIFdel{underlies }\DIFdelend \DIFaddbegin \DIFadd{underlie }\DIFaddend most existing bedload diffusion models.
