...
Landscapes evolve when water, wind, and ice, driven by gravity, erode the uplifted relics of forces from below Earth's surface.
...
Earth's biota colonize these networks which form conduits for migration, transmitting water, genetic information, and organic material alike, while biota convert sediments to soils, staging the joint evolution of life and landscapes which has occurred across geological time.
...
In this context river geomorphology as a scientific discipline is shifting toward more quantitative methods that enable concrete predictions about the natural world.
Sediment transport in river channels is especially amenable to this quantitative approach since it is basically the result of fluid and granular physics \citep{Frey2011}.
Sediment moves in different modes depending on the relative importance of the fluid forces against the weight of grains.
When particles are coarse as in gravel-bed rivers, the fluid forces are relatively weak and particles move as ``bedload", by bouncing, rolling, and sliding along the bed surface. In bedload transport conditions, fluid turbulence and the irregular bed surface inherit special control over sediment dynamics.

Bedload transport exerts particular influence over channel morphology and stability \citep{Church2006,Hassan2008,Recking2016}, in part because the coarsest grains in a river provide a partially-immobile skeleton upon which sedimentary deposits can develop \citep{Zimmerman2007,Comiti2012, McKenzie2018, Eaton2020}.
...
Unfortunately, existing approaches to compute the bedload flux are inadequate, with predictions regularly deviating from measured values by orders of magnitude \citep{Gomez1989, Barry2004, Bathurst2007a, Recking2012, Dhont2018}.
Considering that this is despite well over a century of research into bedload transport \citep{Gomez1991}, we can conclude that new research approaches are sorely needed \citep{Ancey2020a,Ancey2020}.
...
Bedload fluxes are thus in a cyclical feedback with their controls in the details of turbulence and bed organization \citep{Jerolmack2005}. 
...
This departs from the traditional strategy of correlating transport rates to the mean flow and sedimentary characteristics, which considers the granular phase as a continuum \citep{Gilbert1917,MeyerPeter1948}.
Since the trajectories of individual grains are governed by Newtonian mechanics, this alternative perspective provides traction on the problem.
This Newtonian approach is nonetheless complicated because the forces driving and resisting sediment motion vary through space and time, in part due to the turbulent flow and the erratic interactions of moving particles with the bed.
...
The thesis begins with a review of the most relevant pieces of this earlier literature on which my research builds. This review rephrases works when necessary to indicate common themes and to show continuity with my own research which follows in the subsequent chapters.
...
Since the bed is a granular surface, its geometry is very difficult to characterize \citep{Gordon1972}, and the outcome of collisions varies from one to the next \citep{Sekine1992}
Entrainment and deposition provide an additional layer of complexity.
Moving particles that encounter the bed with sufficiently low velocities can settle into pockets which protect them from the flow \citep{Miller1966} and enable their deposition \citep{Charru2004}.
These pockets are not permanent shelter, because rearrangement of the surrounding bed by conventional entrainment or subsurface creep \citep{Houssais2016,Frey2014} can re-expose particles to the flow, and sufficiently strong turbulent fluctuations \citep{Cameron2020} can overcome shelter even if the pocket geometry is not disturbed \citep{Valyrakis2012,Celik2014}. 
As a result, particles generally alternate through sequences of entrainment and deposition.
...
Individual trajectories of particles therefore involve a number of contributing processes which occur over different characteristic timescales.
\citet{Nikora2001,Nikora2002} provided a conceptual model of individual particle motions which helps to organize this complexity.
...
This set of timescales -- local, intermediate, global, and geomorphic -- are used below to organize transport models of individual grains.

\subsection{Motivation: tracers and basic understanding}
The original motivation to understand individual particle motions was probably to predict the efficiency of sediment transport measurements \citep{Ettema2004}, and this motivation still drives a great deal of research into individual particle motions today \citep{Hassan2017,Pretzlav2021}.
