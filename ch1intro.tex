...
Landscales change when sediment moves.
...
At first glance, maybe at the idealistic beginnings of one's PhD, this seems an elementary problem in general physics, but then some challenges appear.
...
\citet{Nikora2001,Nikora2002} provided a conceptual model of individual particle motions which helps to organize the challenges and research progress, although it has been slightly revised in more recent studies \citep{Campagnol2013, Hassan2017, Pierce2020}.
...
\section{The bedload transport problem}

In river science, a fundamental problem is the determination of the bulk bedload flux, or the rate of downstream movement of coarse sediment grains \citep{Ballio2014}.
Since bedload transport controls stream morphology \citep{Church2006, Recking2016}, its prediction is useful to a wide range of problems, from aquatic habitat restoration to energy production \citep{Kondolf2014, Wohl2015a}. 
...
Turbulent impulses induce sediment motion \citep{Valyrakis2010, Celik2014, Amir2014, Shih2017}, but moving sediment affects turublence characteristics \citep{Singh2010, Santos2014, Liu2016}. Bedload transport is controlled by the configuration \citep{Miller1966, Paintal1971,} and consolidation \citep{Johnson2016,Allen2018,Masteller2019} of grains on the bed surface, but bedload transport also modifies the stability and arrangement of bed surface grains \citep{Kirchener1990, Charru2004, Hassan2008}.
The bedload flux controls its control variables. It is in a cyclical feedback with the details of turbulence and bed organization \citep{Jerolmack2005}, so predicting it is challenging.
...
\subsection{Queueing theories for scale dependence}

The final approach to summarize is that of \citet{Ancey2020}, which calculates the sediment flux probability distribution from an ``arrival time distribution".
This is the distribution of times between subsequent arrivals of particles to a control surface.
This approach is reminiscient of queueing theory, which evalues the wait times of customers in a line given a distribution of service times \citep{Cox1961}. 

\citet{Ancey2020} considered two different arrival time distributions to calculate the time-averaged sediment flux probability distribution.
The first is a simple exponential distribution, $P(t) = \Lambda \exp(-\Lambda t).$ Here the mean time between particle arrivals is $1/\Lambda$.
In this case, the flux can be represented with a Poisson pulse noise, similar to the particle position in the Einstein model of section \ref{sec:einstein}:
\be q(t) = \frac{1}{t}\int_0^t dt' \mu(t'). \ee
Here, the noise is 
\be \mu(t) = \sum_{i=1}^{N(t)}\delta(t-t_i), \label{eq:poissfluxnoise}\ee
where $N(t)$ is Poisson distributed with rate $\Lambda$. 
This flux is a random variable. Its probability distribution can be derived by evaluating $P(q,t) = \bra \delta(q-\int_0^t \mu(t')/t) \ket$, which involves an average over all possible realizations of the noise in eq. \ref{eq:poissfluxnoise}, providing \citep{VanKampen2007}
\be  \ee
